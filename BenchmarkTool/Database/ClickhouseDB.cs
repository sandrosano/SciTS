    ...
    public class ClickhouseDB : IDatabase
    {
        ...
        public async Task<QueryStatusRead> RangeQueryRaw(RangeQuery query)
        {
            ...
            try
            {
                ...
                return new QueryStatusRead(true, points, new PerformanceMetricRead(sw.Elapsed.TotalMicroseconds, points, 0, query.StartDate, query.DurationMinutes, 0, Operation.RangeQueryRawData));
            }
            catch (Exception ex)
            {
        ...
        public async Task<QueryStatusRead> RangeQueryRawAllDims(RangeQuery query)
        {
            ...
            try
            {
                ...
                return new QueryStatusRead(true, points, new PerformanceMetricRead(sw.Elapsed.TotalMicroseconds, points, 0, query.StartDate, query.DurationMinutes, 0, Operation.RangeQueryRawAllDimsData));
            }
            catch (Exception ex)
            {
                Log.Error(String.Format("Failed to execute Range Query Raw Data on ClickhouseDB. Exception: {0}", ex.ToString()));
